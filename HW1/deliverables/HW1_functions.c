...
void question_4(){
    ...
    // A wait inside a child doesn't seem to do anything. No children to wait for.
}
...
void question_3(){
    ...
    // Useful for fine control when "exec"ing a process with a program. Allows for environment injection.
    //
...
void question_2(){
    ...
    // NOTE that both the parent and child can access the file descriptor. 
    // But open, read, and write to the file separately.
...
void question_1(){
    ...
    // NOTE that the child prints x as 100 initially when run. 
    // This shows that a separate copy of x is created for the child process.
    // COW creates a copy of the original data (only relevant part) for a process on write, but not read.
